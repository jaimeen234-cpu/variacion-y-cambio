...
App web didáctica en Angular (SPA, sin backend). El dominio es puro y determinista, lo que hace que sus pruebas sean aritmética con valores esperados calculables a mano.

### Runner e invocación

| Suite | Comando | Regla |
|---|---|---|
| Todas | `npm run test:agent` | Silencioso en verde; **completo y verbatim en rojo** |
| Un archivo | `npm run test:agent -- --include='**/<archivo>.spec.ts'` | |
| Arquitectura | `npm run test:arch` | Falla si un import cruza una frontera prohibida |
| E2E *(opcional)* | Playwright, solo si está instalado y el spec lo pide | Su ausencia se declara, no se disimula |

> ⚠️ Estos scripts se crean en el spec de bootstrap. Si aún no existen, **repórtalo** en lugar de improvisar un comando.

### Idiomas de prueba por capa

| Capa | Cómo se prueba |
|---|---|
| `src/app/domain/` | Pruebas unitarias planas, **sin `TestBed`**, sin DOM, sin red. Entrada → salida esperada |
| `src/app/application/` | Casos de uso con **adaptadores falsos en memoria**, sin `TestBed`, sin red ni `localStorage` real |
| `src/app/ui/` | `TestBed` con componentes standalone. Se prueba el comportamiento observable, nunca detalles internos |

### Qué cubrir siempre en este dominio

- **Determinismo:** dos corridas idénticas producen resultados idénticos.
- **Restricciones negativas** (`PERO … NO debe`): tienen prueba propia. Ejemplo: con la temperatura oscilando alrededor del umbral, la frecuencia **NO** debe alternar en cada paso (histéresis).
- **Rangos inválidos:** una entrada fuera del rango físico **no** produce resultado; produce un error tipado.
- **Invariantes de frontera:** la frecuencia efectiva nunca sale de `[frecuenciaBase, frecuenciaObjetivo]`.
- **Estado vacío:** un resultado sin eventos de throttling no rompe ni colapsa la vista.
- **Regresión numérica:** valores esperados calculados a mano y escritos en el propio test, nunca copiados de la salida del código que se está probando.
