...
## 📌 Contexto del proyecto — Variación y Cambio

App web didáctica en Angular (SPA, sin backend). Arquitectura hexagonal.

### Qué auditar, y contra qué documento

| Área | Documento que citas en el ítem FAIL |
|---|---|
| Conformidad con el requisito | `requirements.md` del spec (`RF-n.m`) |
| Fronteras de capa, contratos, NFRs, ADRs | [`docs/trd/trd.md`](../docs/trd/trd.md) |
| Tokens de diseño, layout, accesibilidad, responsive | [`docs/ux-ui/design.md`](../docs/ux-ui/design.md) |
| Alcance | `tasks.md` del spec |

### Violaciones de frontera *(las más importantes de este proyecto)*

| Capa | Puede importar de | Nunca importa de |
|---|---|---|
| `src/app/domain/` | solo `domain/` | Angular, RxJS, Chart.js, `application/`, `infrastructure/`, `ui/` |
| `src/app/application/` | `domain/` | Angular, `infrastructure/`, `ui/` |
| `src/app/infrastructure/` | `domain/`, `application/`, Angular | `ui/` |
| `src/app/ui/` | todas | — |

Marca FAIL, citando TRD §4, si detectas: un import prohibido; `signal`/`computed` fuera de `ui/`; `Date.now()` o `Math.random()` en `domain/`; `chart.js` importado fuera de `ui/shared/vc-time-chart`; un caso de uso que depende de un adaptador concreto en vez de un puerto.

### Convenciones del framework a auditar

Componentes standalone · `OnPush` en todos · `inject()` · rutas con `loadComponent` · `provideHttpClient` solo en `infrastructure/http/` · sin `innerHTML` ni `bypassSecurityTrust*` (TRD SEC-B).

### Tokens de diseño

Auditar contra [`docs/ux-ui/design.md`](../docs/ux-ui/design.md) §7. FAIL ante: cualquier hexadecimal fuera de `ui/styles/_tokens.scss`; un color de serie usado para una magnitud distinta a la de §7.3; un número en vivo sin tipografía tabular; scroll horizontal por debajo de 360 px; objetivo táctil de control de simulación menor de 44 px.

### Deuda heredada

Una afirmación `UNVERIFIABLE` heredada de una tarea anterior es algo **que se vuelve a comprobar**, no algo que se acepta. En este proyecto el caso típico es "no se pudo verificar porque los scripts npm aún no existen": si el spec de bootstrap ya se completó, la premisa dejó de valer y la verificación ahora es exigible.
