...
### Convenciones del framework *(obligatorias)*

| Regla | Detalle |
|---|---|
| Componentes | **Standalone** siempre. Sin `NgModule` |
| Detección de cambios | `ChangeDetectionStrategy.OnPush` en todos, sin excepción |
| Reactividad | `signal` / `computed` / `effect` **solo en `ui/`** |
| Inyección | Función `inject()`; los puertos se resuelven por `InjectionToken` |
| Rutas | `provideRouter` con `loadComponent` (carga diferida por página) |
...
| Idioma | Español en nombres de dominio, textos e identificadores de negocio |

### Fronteras de directorio *(no las cruces)*
...
El dominio es **puro y determinista**: sin `Date.now()`, sin `Math.random()`, sin efectos. Chart.js se importa en **un solo archivo**: `ui/shared/vc-time-chart`.

### Tokens de diseño *(cumplimiento obligatorio)*

Todo lo visual sale de **[`docs/ux-ui/design.md`](../docs/ux-ui/design.md) §7**. **Cero hexadecimales sueltos** en cualquier archivo que no sea `ui/styles/_tokens.scss`. La semántica de color por magnitud (§7.3) es regla dura: potencia = `--vc-info`, temperatura = `--vc-danger`, frecuencia efectiva = `--vc-success`, umbral = `--vc-warning` discontinuo. Un color de serie nunca se reutiliza para otra magnitud.

Los números que cambian en vivo usan `--vc-font-mono` con `font-variant-numeric: tabular-nums`.

### Comandos de verificación *(ejecútalos antes de reportar)*

| Propósito | Comando |
|---|---|
| Pruebas | `npm run test:agent` |
| Una sola prueba | `npm run test:agent -- --include='**/<archivo>.spec.ts'` |
| Prueba de arquitectura | `npm run test:arch` |
| Lint | `npm run lint:agent` |
| Build de producción | `npm run build` |

En verde basta la línea de resumen; **en rojo se reporta la salida completa y verbatim** — es evidencia.

> ⚠️ Estos scripts se crean en el spec de bootstrap. Si aún no existen, **declara la verificación como no realizable** y dilo en tu informe. No inventes un comando alternativo ni reportes verde sin haber verificado.
