...
App web didáctica en Angular (SPA, sin backend) sobre el lazo energía → potencia → calor → throttling. Arquitectura hexagonal verificada por prueba automática.

### Fronteras de directorio *(con esto juzgas la independencia entre tareas)*

| Directorio | Contenido | Dependencias permitidas |
|---|---|---|
| `src/app/domain/` | Entidades, value objects, servicios de dominio, **puertos** | Solo `domain/`. **Cero** Angular / RxJS / Chart.js |
| `src/app/application/` | Casos de uso | `domain/` |
| `src/app/infrastructure/` | Adaptadores (JSON HTTP, localStorage, URL) + tokens de DI | `domain/`, `application/`, Angular |
| `src/app/ui/` | Componentes standalone, páginas, rutas, estilos | Todas |
| `src/assets/data/` | Datos de referencia en JSON | — |
| `docs/` | Constitución y specs | — |

**Al evaluar paralelismo:** archivos distintos **no basta**. Este proyecto tiene un único `dist/`, un único servidor de desarrollo en el puerto 4200 y un único `node_modules/`. Dos tareas que compilen o levanten el servidor a la vez colisionan aunque toquen archivos distintos, y el error aparecerá en el agente equivocado. Serializa toda tarea que compile, mida o levante el servidor.

**Señal de tarea que casi siempre es serial:** cualquiera que toque `src/app/ui/pages/` de la misma página, `angular.json`, `package.json`, o los archivos de estilos globales.

**Rutas de la constitución:** `docs/prd.md` · `docs/ux-ui/design.md` · `docs/trd/trd.md` · `docs/infrastructure.md` · `AGENTS.md` (Skill Map, Model Routing, comandos de verificación).
